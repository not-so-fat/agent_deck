...
Deck playbooks are task recipes — check `triggers` on `get_bound_deck` playbooks, then `get_playbook`. Don't mirror into `.cursor/skills/` — one source of truth on the deck.
